...
def pack_json_data():
    try:
        ...
        data_name = filedialog.asksaveasfilename(title='选择保存位置并重命名文件', initialfile='请为点位文件命名',initialdir=cheat_path)
...
def change_game_type():
    try:
        if not os.path.exists('./game_path.ini'):
            set_game_path = filedialog.askdirectory(title='请选择Genshin Impact Game文件夹')
        ...
        if game_path == '':
            ...
            change_game_type()
        if os.path.exists(f'{game_path}/YuanShen_Data/Plugins/metakeeper.dll'):
            gt.set(value='当前为：官服')
...
game_type = Combobox(window, width=16, values=('官服转b服', 'b服转官服', '官服转国际服', '国际服转官服'),state='readonly', font='黑体')
