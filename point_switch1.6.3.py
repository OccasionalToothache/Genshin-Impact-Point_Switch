...
class PointSwitch:
    ...
    def MainUI(self):
        ...
        self.addaccount = tk.Button(master=self.master, text='添加账号', font=('黑体', 14), command=lambda: self.Addaccount(hotkey=None))
        ...
        tooltip.Tooltip(self.duokai, '某些设备可能无法在作弊状态下多开', topmost=True)
        ...
        tooltip.Tooltip(self.sc, '勾选之后需要切换账号才会生效,快捷键F9')
        ...
        try:
            ...
            if not ask == None and not hotkey == 'f12' and not Gametype == 'b服':
                if not scan.get():
                    ...
                    self.add_scan(Gametype, ask)
                    self.Accountsbox['values'] = os.listdir('./scanner')
        except Exception:
            pass

    def add_scan(self, gametype, name):
        try:
            uid = simpledialog.askstring(title='请输入米游社uid', prompt='请输入米游社uid', parent=self.master)
            if not uid == None and not uid == '':
                device = simpledialog.askstring(title='请输入device', prompt='请输入device', parent=self.master)
                if not device == None and not device == ' ':
                    token = simpledialog.askstring(title='请输入账号token', prompt='请输入账号token', parent=self.master)
                    if not token == None and not token == '':
                        with open(f'./scanner/{name}-{gametype}-S', 'w', encoding='utf-8') as f:
                            f.write(f'uid:{uid}\ndevice:{device}\ntoken:{token}')
                            messagebox.showinfo(title='提示', message='添加成功')
        except Exception:
            messagebox.showwarning(title='警告', message='添加失败')
    ...
    def hot(self):
        ...
        keyboard.add_hotkey('f9', lambda: scanner.main(self.sc_account))
        keyboard.wait()
    ...
    def on_closing(self):
        ...
        os.mkdir('./databases')
        for process in psutil.process_iter(['pid', 'name']):
            if process.name() == 'Pointswitch.exe':
                subprocess.call(['taskkill', '/F', '/T', '/PID', str(process.pid)])
            elif process.name() == 'Pointswitch_Web.exe':
                subprocess.call(['taskkill', '/F', '/T', '/PID', str(process.pid)])
