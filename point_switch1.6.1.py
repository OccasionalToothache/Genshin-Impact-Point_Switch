...
class PointSwitch:
    ...
    def MainUI(self):
        ...
        self.ChangePoint.place(x=185, y=240)
        ...
        tooltip.Tooltip(self.duokai, '如果出现进门报错,请取消勾选后再尝试', topmost=True)
    ...
    def Start(self):
        ...
        if os.path.exists(f'{gamepath}/YuanShen.exe') and not startmode.get():
            ...
            subprocess.run([f'{cheat}', '-analog_inject'], cwd=os.path.dirname(cheat), shell=True)
        elif startmode.get():
            subprocess.run(cheat, cwd=gamepath, shell=True)
