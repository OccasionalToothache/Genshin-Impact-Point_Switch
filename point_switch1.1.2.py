...
class PointSwitch:
    def __init__(self, master):
        self.master = master
        self.config = None
        master.title('点位切换器v1.1.2')
        master.geometry('352x340+550+200')
        master.resizable(False, False)
        master.wm_attributes('-topmost', 1)
        master.iconbitmap(default='./data/resources/title.ico')
        self.Menus()
        self.MainUI()
        self.Config()
        self.CheckUpdate()
    ...
    def CheckUpdate(self):
        ...
        if response != '1.1.2':
            if messagebox.askyesno(title='更新', message='检测到最新版本，是否更新'):
                webbrowser.open(url='http://www.pyforme.fun/update/point_switch.exe')
            else:
                sys.exit()
        else:
    ...
    def GameTypeChange(self):
        ...
        if Value == '官服转b服':
            shutil.unpack_archive(filename='./data/game_pack/官转b.zip', extract_dir=gamepath)
            os.remove(f'{gamepath}/YuanShen_Data/Plugins/metakeeper.dll')
            messagebox.showinfo(title='提示', message='已成功从官服转为b服')
        elif Value == 'b服转官服':
            shutil.unpack_archive(filename='./data/game_pack/b转官.zip', extract_dir=gamepath)
            ...
            shutil.unpack_archive(filename='./data/game_pack/官转国.zip', extract_dir=gamepath)
            ...
            shutil.unpack_archive(filename='./data/game_pack/国转官.zip', extract_dir=gamepath)
    ...
    def Config(self):
        ...
        if not os.path.exists('config.ini'):
            ...
            if not self.config.has_option('DEFAULT', 'cheatpath') or not self.config.get('DEFAULT', 'cheatpath'):
                ...
                messagebox.showinfo(title='提示', message='路径设置成功')
        else:
            self.config.read('config.ini')
            gamepath = self.config['DEFAULT']['gamepath']
            self.config.read(f'{gamepath}/config.ini')
            gametype = self.config['General']['cps']
